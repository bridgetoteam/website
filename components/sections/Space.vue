        ...
        <div class="text">
          ...
          >
            <template v-if="$i18n.locale === 'ja'">
              <p>
                戦後に建てられた京都の古い一軒家。目の前には川が走り、このエリアで唯一という個人用の橋がかかっています。2019年に改装・補強をし、アーバニストのクリエイティブレジデンスおよびコミュニティスペースとしてオープンしました。コンパクトなスペースですが、自分達自身の棲家としながら、レジデンスの他、イベント・ギャラリー・ワークショップなど多目的に使用しています。
              </p>
              <p>
                家の一部は自分達で改修をし、DIY精神を大切にするようにしています。また、電力は100%自然エネルギーを使用し、プラスチックや使い捨てのゴミを出さない、ゼロウェイストの生活を心がけています。日本の空き家や中古物件を、できるだけ環境に優しい形でDIYし、日本の不動産のもつ可能性と価値を発信したいと考えています。
              </p>
            </template>
            <template v-else>
              <p>
          ...
          >
            ...
            <template v-if="$i18n.locale === 'ja'">
              <p>
                Bridge To
                Kyotoには滞在用のゲストルームが2階に1部屋あります。建築、街づくり、不動産などに関わるアーバニストを対象に、1度に1名〜2名
                /
                1ヶ月〜での滞在が可能です。ベッド、デスクなど基本的な家具は揃っており、滞在中はコミュニティスペースの自由な利用が可能です。
              </p>
            </template>
            <template v-else>
              <p>
